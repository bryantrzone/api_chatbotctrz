...
//     $file_ch = curl_init();
//     curl_setopt($file_ch, CURLOPT_URL, $download_url);
//     curl_setopt($file_ch, CURLOPT_RETURNTRANSFER, true);
//     curl_setopt($file_ch, CURLOPT_FOLLOWLOCATION, true); // <-- IMPORTANTE
...
function descargarMediaWhatsApp($media_id) {
    ...
    file_put_contents("whatsapp_log.txt", "📄 Iniciando descarga de media ID: $media_id\n", FILE_APPEND);
    file_put_contents("whatsapp_log.txt", "🔗 URL de API: $url\n", FILE_APPEND);
    file_put_contents("whatsapp_log.txt", "📊 Respuesta API WhatsApp: $status - $response\n", FILE_APPEND);
    ...
    // 2️⃣ Paso 2: Descargar el archivo usando el token en el header
    ...
    $file_content = curl_exec($ch2);
    $status2 = curl_getinfo($ch2, CURLINFO_HTTP_CODE);
    $error2 = curl_error($ch2);
    curl_close($ch2);

    file_put_contents("whatsapp_log.txt", "📥 Estado descarga: $status2, Tamaño: " . strlen($file_content) . " bytes\n", FILE_APPEND);

    // Validación básica del contenido
    $primeros_bytes = bin2hex(substr($file_content, 0, 10));
    file_put_contents("whatsapp_log.txt", "🔍 Primeros bytes (hex): $primeros_bytes\n", FILE_APPEND);

    // Si contiene HTML o error, descartar
    if (substr($file_content, 0, 5) === '<!DOC' || substr($file_content, 0, 1) === '{') {
        file_put_contents("whatsapp_log.txt", "⚠️ Archivo no válido detectado (HTML o JSON)\n", FILE_APPEND);
        return false;
    }
